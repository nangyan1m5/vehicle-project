...
public class BikeRegionMR {
    ...
    public void submitJob(String inputPath, String outputPath) throws Exception {
        ...
        Job job = Job.getInstance(conf, "Bike Region Distribution");
        job.setJarByClass(BikeRegionMR.class);
        job.setMapperClass(RegionMapper.class);
        job.setCombinerClass(RegionReducer.class); // 局部合并优化
        ...
        if (!success) {
            throw new Exception("地区车辆分布统计任务失败");
        }
    }
}